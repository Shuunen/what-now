...
describe('AppDataSchema', () => {
  it('A applies defaults for an empty object', () => {
    ...
    expect(data).toStrictEqual({ settings: { coachEnabled: true, coachLanguage: 'en', finaleDismissedOn: '', ollamaUrl: 'http://localhost:11434', syncUrl: '', userName: 'Me', webhook: '' }, tasks: [] })
  })
...
describe('defaultAppData', () => {
  it('A is an empty state', () => {
    expect(defaultAppData).toStrictEqual({ settings: { coachEnabled: true, coachLanguage: 'en', finaleDismissedOn: '', ollamaUrl: 'http://localhost:11434', syncUrl: '', userName: 'Me', webhook: '' }, tasks: [] })
  })
})
...
describe('recoverAppData', () => {
  ...
  it('B falls back to default settings when settings are invalid', () => {
    ...
    expect(data.settings).toStrictEqual({ coachEnabled: true, coachLanguage: 'en', finaleDismissedOn: '', ollamaUrl: 'http://localhost:11434', syncUrl: '', userName: 'Me', webhook: '' })
  })
